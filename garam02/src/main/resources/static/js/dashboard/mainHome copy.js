...
function makeMainBigCal(nowD, day) {
    ...
    function setMainCalendar() {
        return new Promise(function (resolve, reject) {
            ...
            const dayST = toStringByFormatting(stD);
            let dayED = "";

            let tmpMax = 0;
            let mapArrMax = new Array();
                ...
                function plus2(result) {
                    return new Promise(function (resolve, reject) {
                        ...
                        $.ajax({
                            ...
                            success: function (r) {
                                resolve(parseInt(result) + parseInt(r.length));
                            }
                        })
                    })
                }
                    ...
                    if (trNum == 5 && tdNum == 6) {
                        for (var i = 0; i < 42; i++) {
                            ...
                            if (toStringByFormatting(new Date()) == $(bbb4).val()) {
                                $(bbb).attr('class', 'maincalToday tdMainCal');
                            } else {
                                $(bbb).attr('class', 'whiteTd tdMainCal');
                            }

                            let inHtml = ``;

                            for (let i = 0; i < mapArrMax[trNum]; i++) {
                                inHtml += `
                                    ...
                                    if (toStringByFormatting(dayyyy) == $(bbb4).val()) {
                                        ...
                                        switch (r[i].ctmsepa) {
                                            case 0:
                                                ...
                                                break;
                                            case 1:
                                                qwer += `<span class="spNum2">` + r[i].ctmname + `</span>`;
                                                break;
                                            case 2:
                                                qwer += `<span class="spNum1">` + r[i].desty + `</span>`;
                                                break;
                                        ...
                                        switch (r[i].bus) {
                                            case '대형':
                                                qwer += `<span class="spNum2 big45">` + r[i].num + `</span>`;
                                                break;
                                            case '중형':
                                                qwer += `<span class="spNum2 big25">` + r[i].num + `</span>`;
                                                break;
                                            case '우등':
                                                qwer += `<span class="spNum2 big28">` + r[i].num + `</span>`;
                                                break;
                                        }
                                        if (k == 0) {
...
$(document).on('click', '.mainCaltd-top', function () {
    console.log("하이요");
});

$(document).ready(function () {
    $(".mainCalTable tbody tr td .mainCaltd-top").mouseover(function () {
        $(this)
            .parent()
            .parent()
            .addClass('tdHovers');
    });
    $(".mainCalTable tbody tr td .mainCaltd-top").mouseout(function () {
        $(this)
            .parent()
            .parent()
